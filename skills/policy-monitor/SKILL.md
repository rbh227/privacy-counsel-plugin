...
1. Read `../../profile/privacy-playbook.md` → outputs folder path and policy surfaces; read the matter's last sweep date from `~/.claude/privacy-counsel/matters/<matter-slug>/sweep-state.md`. If it is missing, perform a FULL first sweep of that matter, then create the cursor. Never seed a cursor from any legacy global `Last policy sweep` value — sweeps are always per-matter/per-client.
...
6. Update `Last sweep:` in the matter's `sweep-state.md` — only after a successful COMPLETE sweep of that matter; a partial scan never advances the cursor.

**Direct query mode** (with description argument):
1. Read `../../profile/privacy-playbook.md` → current policy commitments + actual policy document.
...
**Schedule:** Set up a recurring reminder in your own scheduler (calendar, task manager, or CI) to run `the policy-monitor skill` weekly. Scheduled execution requires a scheduled-tasks integration, which is not bundled with this plugin.

```
the policy-monitor skill
the policy-monitor skill "We want to start using behavioral data to personalize onboarding emails"
```
...
Read `../../profile/privacy-playbook.md`:
...
If the matter does not identify the policy surfaces to sweep:
...
> the crawl sweep, tell me the client's policy URL and any other surfaces
> folder path."

Read the actual privacy policy document from the path in `## Outputs` → **Privacy
policy document**. The commitments in the playbook/matter record are a summary; the actual document
...
> - [regime]: [notice path + last updated, or "UNKNOWN — ask the client; flag each sweep until resolved"]
...
Read the matter's `sweep-state.md` → `Last sweep:` date. Scan for output files in the
...
Update `Last sweep:` in the matter's `sweep-state.md` to today's date — only after a successful complete sweep of that matter.
...
- Match the voice and style of the existing policy (read the actual document, not
  just the summary, before drafting)
...
to run `the policy-monitor skill` weekly. Scheduled execution requires a
scheduled-tasks integration, which is not bundled with this plugin.

Whenever the sweep runs, it updates `Last sweep:` in that matter's
`sweep-state.md`, so the next sweep of THAT matter only looks at new files.
A first sweep of a different matter always starts from the beginning —
another matter's cursor never applies.
...
If the sweep surfaced more than ~10 drift findings, or any time the user asks: offer the dashboard (see `../../profile/privacy-playbook.md` `## Outputs` → Dashboards). Shape the offer for this output — counts by surface (policy clause / PIA / DPA / triage), counts by severity, and a sortable grid of findings with source artifact and recommended remediation.
