...
description: >
  ...
  assessment in `../../profile/privacy-playbook.md`.
...
1. Read `../../profile/privacy-playbook.md`. Confirm impact assessment house style is populated.
...
6. Policy diff against `../../profile/privacy-playbook.md` AI policy commitments.
7. Output: assessment doc + conditions list + handoff flags (privacy PIA, vendor review if needed).

```
the aia-generation skill "AI résumé screening for HR"
```
...
conversation and writes the output in the house format — see the playbook's
`## PIA house style`, which covers impact assessments beyond privacy.
...
- Is the use case in the elevated or high governance tier per `../../profile/privacy-playbook.md`?
...
Before intake, determine which track to run. The tier definitions and the fast-track criteria come from `../../profile/privacy-playbook.md` (`## Use case registry` and `## Governance tiers`), not from any hardcoded regime-specific framework.
...
**Fast track vs. full assessment:** `../../profile/privacy-playbook.md` defines what qualifies for abbreviated treatment. If `../../profile/privacy-playbook.md` doesn't define fast-track criteria, default to full assessment and ask the user what criteria they want captured for next time.
...
**Step 3 pre-check — footprint freshness.** Before iterating over the captured `## Regulatory footprint`, compare the use case's affected population and decision type (from Step 2) against the footprint as written. The playbook's footprint is a practice-wide default, not a per-client finding. If the use case introduces an affected population (e.g., children, employees in a new state, EU data subjects) or a decision type (e.g., hiring, creditworthiness, health diagnosis, law enforcement, critical infrastructure) that the footprint does not contemplate, **re-derive the applicable regimes rather than iterating over the stale list.**

Say to the user:

> "The practice profile's regulatory footprint was set for [the affected populations and decision types the practice footprint contemplates]. This use case affects **[new population or decision type — e.g., employees in Colorado, minors under 13, credit decisions, biometric identification]**, which is not in the captured footprint. I'm going to re-derive the applicable regimes from the CLIENT's operating jurisdictions (from the active matter's `matter.md`) and this use case's decision type ([Y]), rather than use the stale footprint. If this use case is representative of work you expect to see more of, record the wider footprint in the ACTIVE MATTER's overrides, not the practice ones. The regimes you just derived came from THIS client's jurisdictions and THIS use case; promoting them to a practice default would hand the next client another client's sectoral posture, and the next matter would inherit it silently because a practice override outranks the playbook. Only write `footprint/regimes` at practice level if the partner states plainly that this is a firm-wide default independent of this client — and say that is what you are doing before you write it."
...
For each regime in `../../profile/privacy-playbook.md` → `## Regulatory footprint` that applies to this system — **plus any regime surfaced by the re-derivation above** — research the currently operative risk classification framework and determine where the system lands.
...
**Use the seed structure from `../../profile/privacy-playbook.md`.** If none was captured, use this default:
...
|---|---|---|
| [commitment from `../../profile/privacy-playbook.md` AI policy section] | 🟢 / 🟡 / 🟠 / 🔴 | |
...
**Privacy review required?** [Yes — run `the pia-generation skill` /
No]
...
**Before certifying the AIA (the Sign-off step, marking Status: APPROVED):** Read `## Who's using this` in `../../profile/privacy-playbook.md`. If the Role is Non-lawyer:
...
Every assessment should cross-check against the AI policy commitments in `../../profile/privacy-playbook.md`.
Common drift:
...
- **To privacy:** If personal data is involved, flag: "Run `the pia-generation skill` in parallel — the AIA doesn't substitute for a PIA."
