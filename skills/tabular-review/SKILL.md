...
1. Load `../../profile/privacy-playbook.md` → diligence structure, thresholds, house format.
...
7. Output: markdown + `.csv` + `_sources.csv` always, to the matter's outputs. A spreadsheet only where the session can genuinely write one (see `## Available integrations`). Work-product header.
...
Triggered from plain language — "review these 30 DPAs for audit rights,
training carve-outs and transfer terms", "build me a comparison table across
this folder", "batch review the VDR contracts folder". There are no flags;
take the same information from the request, and ask only for what is
missing:

- **An existing schema.** If the partner points at a `.review-schema.yaml`
  from an earlier run, use it rather than rebuilding — that is what makes
  re-runs and incremental additions cheap.
- **A template.** `references/ma-diligence-columns.md` is the starting
  column set for M&A diligence; offer it when the ask is deal-shaped.
- **The document source.** A local folder or a VDR path. Ask if absent.
- **Sample size** for the schema check. Five documents unless told
  otherwise.
...
- `../../profile/privacy-playbook.md` → diligence structure, materiality thresholds, house format preferences
- the active matter's `matter.md` if working a specific deal
...
3. **Output.** Markdown + `.csv` + `_sources.csv` to the matter's outputs folder, always. Offer a spreadsheet only if this session can actually write one (`## Available integrations`) — an offer you cannot honor costs the partner a round-trip and their confidence in the rest of the run.
...
**Markdown + CSV always. A spreadsheet only if the session can actually
produce one.** Check `../../profile/privacy-playbook.md` →
`## Available integrations`: document storage is off by default here, so the
default output is markdown plus `.csv` and `_sources.csv` written to the
matter's outputs folder — never an offer to produce a `.xlsx` the session
cannot write, and never a claim that a file landed somewhere it did not.

Where Excel or Sheets IS available in the session (Claude in Excel or an
`openpyxl` path for `.xlsx`; a Sheets MCP or API for Sheets), ask which the
team works in and follow the workbook structure in
`references/excel-output.md` and `references/gsheets-output.md`. Everything
below describes that spreadsheet form; the markdown fallback keeps the same
columns, the same states, and the same source citations, losing only the
hover-and-color affordances.
...
- A disclosure schedule is a filtered, reformatted view of a tabular review — build it from this skill's output rather than re-reading the documents.
- Where the client's deal team runs a dedicated review platform (Luminance, Kira), this skill is the option for anything it can handle; hand off the residue and say which documents went where.
