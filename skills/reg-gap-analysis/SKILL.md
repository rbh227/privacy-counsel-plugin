...
1. Load `../../profile/privacy-playbook.md` → privacy policy commitments, regulatory footprint, DSAR systems.
...
```
the reg-gap-analysis skill "Colorado Privacy Act"
```

```
the reg-gap-analysis skill
...
This skill diffs the new requirement against what you currently do (per `../../profile/privacy-playbook.md` → Privacy policy commitments + the practices documented in PIAs) and produces a gap list with a remediation plan.
...
**We currently:** [what the matter documents / privacy policy / practice shows]
...
**Proactive monitoring:** this skill is manual — it runs when pointed at a change. (Only if a regulatory-monitoring plugin is actually installed does automatic triggering exist; never reference it otherwise.)
