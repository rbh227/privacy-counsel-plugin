...
1. Load `../../profile/privacy-playbook.md` → DSAR process (systems list, verification method, SLA).
...
Read `../../profile/privacy-playbook.md` → `## DSAR process`. That section has:
...
Per the method in `../../profile/privacy-playbook.md`. Common approaches:
...
Walk the systems list from `../../profile/privacy-playbook.md`. For each system:
...
**Before proceeding to send either letter to the data subject:** Read `## Who's using this` in `../../profile/privacy-playbook.md`. If the Role is Non-lawyer:
...
> **Note:** Both DSAR letters are externally-facing deliverables sent to the data subject. Do **not** include the work-product header from `../../profile/privacy-playbook.md` `## Outputs` on either letter. Internal notes, logs, and exemption analyses that accompany the letters are attorney work product — keep those separate and prepend the work-product header per `../../profile/privacy-playbook.md` `## Outputs` (which differs by user role — see `## Who's using this`).
...
Per `../../profile/privacy-playbook.md` → Escalation table, escalate when:
...
If `../../profile/privacy-playbook.md` → `## DSAR process` records an internal SLA that is tighter than the legal deadline, use the internal SLA and note the legal backstop.
