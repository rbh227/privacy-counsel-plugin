...
1. **Read the playbook.** Read `../../profile/privacy-playbook.md`
   (`## AI governance` for tiers, `## Regulatory footprint` for regimes) and
   the practice overrides file. It is always configured — never prompt for
   setup. The "company" whose systems these are is the CLIENT of the active
   matter, not the firm: read the matter's `matter.md` for their role,
   sector, and footprint, and ask only when the answer changes the
   classification.

2. **Read the inventory.** It lives at the active matter's
   `ai-systems.yaml` (see `## Matter workspaces`). If it doesn't exist,
   create it with an empty `systems:` list when the first `add` runs.

   **Say the scope out loud when you create one.** An AI register is a
   CLIENT asset — it outlives any single engagement — but storage here is
   per matter, because the confidentiality wall runs between matters and
   this skill must not read across it. The consequence is real: a second
   matter for the same client starts from an empty register while the
   first one sits invisible next door, and nothing about the empty file
   says so. So on creating a new inventory, tell the partner in one line
   that this register covers this matter only, and ask whether an existing
   register for this client should be carried over. They can hand it
   across; you cannot go and fetch it.

   Never present a matter's register as "the client's AI systems" — it is
   the systems recorded in THIS matter. A classification question answered
   from a partial register is worse than one answered from an empty one.
...
"just ask me to classify <id> later"."
...
2. "Want to run `the aia-generation skill` to produce a full
   impact assessment?"
...
`the aia-generation skill` for the formal impact assessment
if needed.
