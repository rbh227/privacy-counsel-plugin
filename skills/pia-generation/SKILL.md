...
name: pia-generation
description: >
  Generate or continue a Privacy Impact Assessment / DPIA that has already
  been determined necessary. Use ONLY on explicit requests to create,
  draft, or continue an assessment ("generate the PIA", "draft the DPIA",
  "continue the assessment"), or on handoff from use-case-triage after its
  classification. Questions about WHETHER an assessment is needed ("do we
  need a PIA", "privacy check on X", bare activity descriptions) belong to
  use-case-triage, not here.
...
1. Load `../../profile/privacy-playbook.md` → PIA house style (trigger, structure, depth, sign-off).
...
```
the pia-generation skill "Location sharing feature"
```

```
the pia-generation skill
PRD: [Drive link]
```
...
A PIA is a conversation with the product team, captured. It asks: what data, why, how long, who sees it, what could go wrong. This skill structures that conversation and writes the output in the playbook's PIA house style (matter-provided templates override).
...
Before writing a new PIA, check the outputs folder for prior work on the same feature, processing activity, or counterparty. Read `../../profile/privacy-playbook.md` → `## Outputs` for the path. Scan for:
...
**Carry severity from upstream as a floor** per the cross-skill severity floor rule in `../../profile/privacy-playbook.md` → `## Shared guardrails`. A use-case-triage that rated the activity high-risk cannot become a PIA that concludes low-risk without stating why and what changed.
...
Read `../../profile/privacy-playbook.md` → `## PIA house style`. That has:
...
If the matter provides a client PIA template, **use it**. The point is that this PIA looks like the other PIAs this team produces, not like a generic one.

## Step 0: Is a PIA needed?

Check the trigger criteria in `../../profile/privacy-playbook.md`. That is the team's house answer.
...
**Use the matter's client PIA template when provided.** Otherwise use this default. Prepend the work-product header from `../../profile/privacy-playbook.md` `## Outputs` (it differs by user role — see `## Who's using this`).
...
|---|---|---|
| [commitment from the client's privacy policy (matter documents)] | 🟢 / 🟡 | |
...
Every PIA should cross-check against the privacy policy commitments in `../../profile/privacy-playbook.md`. The common drift:
...
- **To the sign-off process:** Per `../../profile/privacy-playbook.md` → who approves PIAs.
...
**Before proceeding to submit a DPIA (or any equivalent impact assessment) to a regulator, supervisory authority, or enforcement body:** Read `## Who's using this` in `../../profile/privacy-playbook.md`. If the Role is Non-lawyer:
